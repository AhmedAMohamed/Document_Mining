 ...
 * only add a function to call the class stemming finction just an interface
 */
/*
...
class Stemmer {
	...
	/**
	 * call the program for demonstrating the Stemmer. It reads text from a a
	 * list of files, stems each word, and writes the result to standard output.
	 * Note that the word stemmed is expected to be in lower case: forcing lower
	 * case must be done outside the Stemmer class. Usage: Stemmer file-name
	 * file-name ...
	 */
	public static HashMap<String, File> getStemedFiles(
			HashMap<String, File> filesToStem) {
		char[] w = new char[501];
		Stemmer s = new Stemmer();
		HashMap<String, File> filesAfterStemming = new HashMap<String, File>();
		for (String fileName : filesToStem.keySet()) {
			try {
				FileInputStream in = new FileInputStream(fileName);
				File file = new File("doc-after-stemming" + fileName);
				...
				filesAfterStemming.put(fileName, file);
