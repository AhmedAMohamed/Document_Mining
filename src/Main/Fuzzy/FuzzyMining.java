...
public class FuzzyMining {
     ...
     *
     */
    public static ArrayList<Cluster> mineFrequentItemSets(ArrayList<DocumentTermFrequency> documents, HashMap<String,
            WordInfo> wordsVector) {
        ...
        // Apriori
        Watch.lapBegin();
        ArrayList<Cluster> candidate_clusters = Apriori.generateAllCandidateClusters(l1_candidate_clusters,
                documents, wordsVector);
    ...
    /**
     * Calculate the fuzzy variables value for all terms in each document and calculate the summed values for
     * each terms in the unified words vector.
     */
    private static void calculateFuzzyVariables(){
        ...
        for(DocumentTermFrequency d : documents) {
            for(String word : wordsVector.keySet()) {
                ...
                double l = Membership.low(d.getWordFreq(word), w.getMinFreq(), w.getAverage(), w.getMaxFreq());
                double m = Membership.mid(d.getWordFreq(word), w.getMinFreq(), w.getAverage(), w.getMaxFreq());
                double h = Membership.high(d.getWordFreq(word), w.getMinFreq(), w.getAverage(), w.getMaxFreq());
                d.setFuzzyValue(word, l, m, h);

                //step 2 calculate summed membership values for term in words vector
    ...
    /**
     * Go over each term in the unified term vector and set its maximum fuzzy variable which is the variable with the
     * highest summed value over all documents. Also produce the L1 which is candidate clusters of size 1.
     *
    ...
    private static ArrayList<Cluster>  calculateMaxFuzzyVariableAndL1(){
        ...
        FuzzyVariable [] variables = {FuzzyVariable.LOW, FuzzyVariable.MEDIUM, FuzzyVariable.HIGH};
        for(String word : wordsVector.keySet())
        {
            WordInfo w = wordsVector.get(word);
