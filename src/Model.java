...
public class Model {
    ...
    private void writeOutput(ArrayList<String> globalWords, ArrayList<DocumentTermFrequency> docsWithFreq) {
        try {
            PrintWriter writer = new PrintWriter(outputDirectory+"/preprocess_output.txt", "UTF-8");
            //first line words
            ...
            for(DocumentTermFrequency doc : docsWithFreq)
            {
                ...
                for(int i = 0; i< globalWords.size(); i++)
                {
                    writer.print(center(String.valueOf(doc.getWordFreq(globalWords.get(i))),
                            getColLenght(i)));
    ...
    private ArrayList<DocumentTermFrequency> createUnifiedTermFrequency(ArrayList<Document> documents) {
    ...
    private void unifyVectoreSpace(ArrayList<DocumentTermFrequency> docsFreq, HashMap<String,
            HashSet<DocumentTermFrequency>> globalWordToDouments) {
